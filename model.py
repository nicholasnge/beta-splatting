...
class BetaModel:
    ...
    def load_png(self, path):
        ...
        if self.sb_number:
            ...
            for i in range(self.sb_number):
                ...
                sb_params_list.append(np.concatenate([color, direction], axis=1))
            sb_params = np.stack(sb_params_list, axis=2)
        else:
            sb_params = np.zeros((xyz.shape[0], 6, self.sb_number))

        self._xyz = nn.Parameter(torch.tensor(xyz, dtype=torch.float, device="cuda").requires_grad_(True))
        self._sh0 = nn.Parameter(torch.tensor(sh0, dtype=torch.float, device="cuda").contiguous().requires_grad_(True))
        self._shN = nn.Parameter(torch.tensor(shN, dtype=torch.float, device="cuda").contiguous().requires_grad_(True))
        ...
        self._scaling = nn.Parameter(torch.tensor(scaling, dtype=torch.float, device="cuda").requires_grad_(True))
        self._rotation = nn.Parameter(torch.tensor(rotation, dtype=torch.float, device="cuda").requires_grad_(True))
        self._sb_params = nn.Parameter(torch.tensor(sb_params, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
    ...
    def densification_postfix(
        self, new_xyz, new_sh0, new_shN, new_sb_params,
        new_opacities, new_betas, new_scaling, new_rotation, reset_params=True,
    ):
        d = {
            "xyz": new_xyz, "sh0": new_sh0, "shN": new_shN, "sb_params": new_sb_params,
            "opacity": new_opacities, "beta": new_betas, "scaling": new_scaling, "rotation": new_rotation,
        }
    ...
    def _update_params(self, idxs, ratio):
        ...
        return (
            self._xyz[idxs], self._sh0[idxs], self._shN[idxs], self._sb_params[idxs],
    ...
    def relocate_gs(self, dead_mask=None):
        ...
        (
            ...
            self._scaling[dead_indices], self._rotation[dead_indices],
    ...
    def add_new_gs(self, cap_max):
        ...
        (new_xyz, new_sh0, new_shN, new_sb_params, new_opacity, new_beta, new_scaling, new_rotation) = (
